...
if [ ${classes_per_dataset} -gt 0 ]; then
  if [ ${wu_epochs} -gt 0 ]; then
    exp_name="${tag}:lamb_${lamb}:ta:mc:wu:wu_wd_${wu_wd}"
    result_path="results/${tag}/lwf_ta_mc_wu_${lamb}_${wu_wd}_${seed}"
    ...
    exp_name="${tag}:lamb_${lamb}:ta:mc"
    result_path="results/${tag}/lwf_ta_mc_${lamb}_${seed}"
    python3 src/main_incremental.py \
      --exp-name ${exp_name} \
      --gpu ${gpu} \
      --datasets ${dataset} \
