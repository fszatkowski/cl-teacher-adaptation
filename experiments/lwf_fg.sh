...
if [ ${classes_per_dataset} -gt 0 ]; then
  if [ ${wu_epochs} -gt 0 ]; then
    exp_name="${tag}:lamb_${lamb}:base:wu:wu_wd_${wu_wd}}"
    result_path="results/${tag}/lwf_base_wu_${lamb}_${wu_wd}_${seed}"
  ...
  if [ ${wu_epochs} -gt 0 ]; then
    exp_name="${tag}:lamb_${lamb}:base:wu:wu_wd_${wu_wd}"
    result_path="results/${tag}/lwf_base_wu_${wu_wd}_${lamb}_${seed}"
    python3 src/main_incremental.py \
      --exp-name ${exp_name} \
      --gpu ${gpu} \
      --datasets ${dataset} \
